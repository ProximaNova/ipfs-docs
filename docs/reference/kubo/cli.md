...
::: tip Generated on 2025-08-27 17:41:00, from kubo 0.36.0
This document was autogenerated from CLI help text in [kubo 0.36.0](https://github.com/ipfs/kubo/releases/tag/v0.36.0)
...
SYNOPSIS
  ipfs add [--recursive | -r] [--dereference-args] [--stdin-name=<stdin-name>]
           ...
           [--chunker=<chunker> | -s] [--raw-leaves]
           [--max-file-links=<max-file-links>]
           [--max-directory-links=<max-directory-links>]
           [--max-hamt-fanout=<max-hamt-fanout>] [--nocopy] [--fscache]
...
OPTIONS
  ...
  --raw-leaves               bool   - Use raw blocks for leaf nodes. Default:
                                      Import.UnixFSRawLeaves.
  --max-file-links           int    - Limit the maximum number of links in
                                      UnixFS file nodes to this value.
...
EXPERIMENTAL SUBCOMMANDS
  ipfs log tail - Read and outpt log messages.
...
EXPERIMENTAL SUBCOMMANDS
  ipfs stats reprovide - Returns statistics about the node's reprovider system.

DEPRECATED SUBCOMMANDS
  ipfs stats provide - Deprecated command, use 'ipfs stats reprovide' instead.
